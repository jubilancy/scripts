...
export default function Sidebar() {
  ...
  const SOCIAL_LINKS = [
    { title: "Email", url: "mailto:hi@rishimohan.me", icon: <Envelope /> },
    { title: "Twitter", url: "https://twitter.com/thelifeofrishi", icon: <TwitterLogo /> },
    { title: "Instagram", url: "https://instagram.com/thelifeofrishi", icon: <InstagramLogo /> },
    { title: "LinkedIn", url: "https://linkedin.com/in/thelifeofrishi", icon: <LinkedinLogo /> },
    { title: "Github", url: "https://github.com/rishimohan", icon: <GithubLogo /> },
  ];

  const LINKS = [
    { title: <div className="md:w-5 md:h-5 flex items-center justify-center"><House size={16} className="hidden md:flex" /><span className="flex md:hidden">Home</span></div>, url: "/", icon: <House size={16} />, active: pathname === "/" },
    { title: "Blog", url: "/blog", icon: <Note size={16} />, active: pathname.includes("/blog") },
    { title: "Pen", url: "/pen", icon: <Note size={16} />, active: pathname.includes("/pen") },
    { title: "Projects", url: "/projects", icon: <BracketsCurly size={16} />, active: pathname.includes("/projects") },
    { title: "Travel Map", url: "/map", icon: <Compass size={16} />, active: pathname === "/map" },
    { title: "Resume", url: "https://peerlist.io/rishimohan", icon: <IdentificationCard size={16} />, active: false, external: true },
  ];

  const RenderLinks = ({ sectionItems }) => (
        ...
        <aside className="sticky top-[30px] overflow-auto flex text-sm px-4 py-2 rounded-[12px] mt-2 w-full">
          ...
          <div className="flex gap-3 items-center ml-auto">
            {SOCIAL_LINKS.map((item) => (
              <Link key={item.url} className="text-lg opacity-50 hover:opacity-80" href={item.url} target="_blank">
                {item.icon}
              </Link>
            ))}
          </div>
      ...
      <AnimatePresence>
        {mobileNav && (
          <nav className="fixed bottom-0 left-0 z-10 block w-full p-2 md:hidden h-full">
            <div className="absolute inset-0 bg-black/20 dark:bg-black/50 w-full h-full" onClick={() => showMobileNav(false)} />
            <motion.div
              initial={{ y: 50 }}
              animate={{ y: 0 }}
            ...
            >
              <div onClick={() => showMobileNav(false)}>
                <div className="absolute top-[-50px] right-[10px] z-20">
                  <Button variant="secondary" className="w-full !py-2" onClick={() => showMobileNav(false)}>Close</Button>
                </div>
                <RenderLinks sectionItems={LINKS} />
